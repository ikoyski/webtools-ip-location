pipeline {
    ...
    environment {
        ...
        DOCKERHUB_IMAGE = 'ikoyski/webtools-ip-location:latest'        
        DEPLOYMENT_FILENAME = 'Deploy-webtools-ip-location-private.yaml'
    }
    stages {
        stage('Git Stuff') {
            steps {
                script {
                	checkout scmGit(
                		...
                		extensions: [], 
                		userRemoteConfigs: [[url: 'https://github.com/ikoyski/webtools-ip-location.git']]
                	)
                }
            }
        }
